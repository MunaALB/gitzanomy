    ...
    <div class="content">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title">عدد الاشتراكات المختارة <span id="count">0</span> <span class="paynow-part"><a style="cursor:pointer;" class="text-primary" onclick="payNow(this);">ادفع الآن</a></span></h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <?php
                                    ...
                                    <div class="headingparts">
                                        ...
                                        <div class="mytoggle">
                                            ...
                                            <?php else: if ($category['selected'] == 2): ?>
                                                    <p class="text-danger">منتهية الصلاحية</p>
                                                <?php else: ?>
                                                    <!--                                                    <label class="switch">
                                            ...
                                            foreach ($category['plan_list'] as $plan): $plan['original_price'] = $plan['price'];
                                                ...
                                                <div class="col-lg-4">
                                                    <div class="card text-center plan-designs">
                                                        <div class="card-body">
                                                            <h4 class="card-title"><?= $plan['name_ar'] ?></h4>
                                                            <h5><?= $plan['price'] ? $plan['price'] . ' LYD' : 'مجانا' ?></h5>
                                                            ...
                                                            if ($plan['price']) {
                                                                if ($category['selected'] == 1) {
                                                                    ...
                                                                    ?>
                                                                    <a style="cursor:pointer" onclick="selectPlan(this,<?= $count ?>);" class="btn btn-primary text-white" data-type="1" data-plan-id="<?= $plan['plan_id'] ?>" data-category-id="<?= $plan['service_category_id'] ?>">اختر الاشتراك</a> 
                                                                    <?php
                                                                }
                                                            } else {
                                                                if (($category['selected'] == 1) && $plan['selected'] == 1) {
                                                                    ?> 
                                                                    <a style="cursor:not-allowed" title="Already Subscribed" class="btn btn-primary text-white disabledplan">تم الاشتراك</a> 
                                                                <?php } else if ($plan['selected'] == 2) { ?>   
                                                                    <a style="cursor:not-allowed" class="btn btn-primary text-white selectedFree">لا ينطبق</a> 
                <?php } else if ($category['selected'] == 1) { ?>
                                                                    <a style="cursor:not-allowed" class="btn btn-primary text-white disabledplan">لا ينطبق</a> 
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <div class="messege-box">
                        ...
                        <h3>هل أنت متأكد من المتابعة</h3>
                    </div>
                    <div class="action-button">
                        <button type="submit" class="btn btn-primary mybtns">نعم</button>
                        <button type="submit" class="btn btn-primary mybtns" data-dismiss="modal">لا</button>
                    </div>
                </div>
            </div>
        </div>
...
//            $(obj).css('background-color', '#f74f00');
    ...
    function payNow(obj) {
        var count = $(".card-body").find(".selected").length;
        if (count) {
            var cnfm = confirm('هل أنت متأكد من أنك تريد متابعة الدفع؟');
