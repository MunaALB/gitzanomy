                ...
                <?php if ($order['order_items']): ?>
                    <?php   
                        ...
                        if($order['isdeliveredProduct']):
                            $readyForDelivery='<a href="'.base_url('admin/assigned-driver-for-user-product/'.$order['order_id']).'" title="Ready for delivery" class="composemail assign-driver pull-left">Ready for delivery</a>';
                        else:
                            ...
                            $readyForDelivery='<a style="background:gray;cursor:no-drop;"  title="Ready for delivery" class="composemail assign-driver pull-left">Ready for delivery</a>';
                            $readyForDelivery='<a href="#" onclick="markAsComplete(this,'.$order['order_id'].');" title="Mark as complete" class="composemail assign-driver pull-left">Mark as complete</a>';
                        endif;
                    ?>
                            ...
                            <div class="card-body">
                                    ...
                                    <div class="col-lg-5">
                                        <div class="user-img pull-left">
                                            ...
                                            <?php else: ?> 
                                                ...
                                                <?php else: ?> 
                                                        <?php if($items['vendor_id']==0): ?>
                                                            <?php if($items['international_driver_id']==0): ?> 
                                                                <span class="sold-out-overlay-hub">Not Assigned</span>
                                                            <?php else: ?>
                                                                <span style="cursor:pointer;" onclick="receivedOnHub(this,'<?=$items['order_id']?>','<?=$items['international_driver_id']?>','<?=$items['international_tracking_id']?>','1');" class="sold-out-overlay-assigned">Assigned Driver</span>
                                                            <?php endif; ?>
                                                        <?php else: ?>
                                                            <?php if($items['driver_id']==0): ?> 
                                                                <span class="sold-out-overlay-hub">Not Assigned</span>
                                                            <?php else: ?>
                                                                <span style="cursor:pointer;" onclick="receivedOnHub(this,'<?=$items['order_id']?>','<?=$items['driver_id']?>','<?=$items['vendor_tracking_id']?>','2');" class="sold-out-overlay-assigned">Assigned Driver</span>
                                                            <?php endif; ?>
                                                        <?php endif; ?>
                                                <?php endif; ?>
                                            <?php endif; ?>
                                ...
                                <?php  }else { if($items['drop_item_status_count']>0): ?>
                                            <div class="status-part" id="item_id_1_<?=$items['order_item_id'];?>">
                                                ...
                                                <ul>
                                                    <?php $width=30; 
                                                    if(isset($items['drop_item_status_count']) and $items['drop_item_status_count']):
            ...
            if(i){
                ...
                if (r == true) {
                    $.ajax({
                        url: "<?= base_url(); ?>admin/Order/ajax_method",
                        type: 'post',
                        data: 'method=markAsComplete&orderId=' + i,
                        success: function (data) {
                            var dt = $.trim(data);
                            var jsonData = $.parseJSON(dt);
                            if (jsonData['error_code'] == "100") {
