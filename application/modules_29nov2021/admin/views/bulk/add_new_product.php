    ...
    <div class="content eventdeatil">  
        <div class="card">
            <div class="card-header">
                <a href="<?=base_url('admin/edit-bulk-upload-product');?>" style="float: right;color: #fff;cursor: pointer;">Edit From Excel</a>
                                ...
                                <?php $lastKeyOfRow=0; $lastKeyOfRowCount=1; $dataColor="#08a3a2"; if ($product_list): foreach ($product_list as $key=>$list):
                                    ...
                                    <?php if($list['error']): 
                                        ...
                                        <tr style="<?=$classData;?>">
                                            ...
                                            <td><?php if($list['hub_error']){
                                                        echo '<i class="fa fa-dot-circle-o" title="Required Field" aria-hidden="true" style="color: red;position: absolute;margin: -30px 43px"></i>';
                                                    } ?>
                                                <?= $list['hub_name'];?></td>
...
function showErrorMessage(id,msg){
    ...
    function generateExcel(o){
        ...
        if(category && sub_category){
            ...
            $.ajax({
                url: "<?php echo base_url("/admin/Bulk/generate_excel") ?>",
                type: "POST",
                data: form_data,
    ...
    function uploadData(o){
        ...
        $.ajax({
                url: "<?php echo base_url("/admin/Bulk/uploading_data") ?>",
                type: "POST",
                data: form_data,
                enctype: 'multipart/form-data',
                cache: false,
                ...
                success: function (data) {
                    //alert(data)
                    if(data=='success'){
                        window.location.href="<?=base_url('admin/bulk-upload-product');?>";
                    }else{
