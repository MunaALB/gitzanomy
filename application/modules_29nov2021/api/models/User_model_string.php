...
//                print_r($productAttribute);EXIT;
    ...
    function getHomeViewAllBooking($data){
        $topBookingServiceArr= array();
        $topBookingService = $this->db->select("service.*,sc.name as category_name,ssc.name as sub_category_name")
                        ->where("service.status",1)
                        ->where("v.status",1)
    ...
    function getCategorySubCategory($data){
        ////////////////////////////CATEGORY/////////////////////////
        $catArr=array();
        $this->db->where('status',1);
        $productCategory = $this->db->get('product_category')->result_array();
        ...
        ////////////////////////////CATEGORY/////////////////////////
        ////////////////////////////Brand/////////////////////////
        $brand = $this->db->select("brand.*")
                        ->where("bm.sub_category_id",$data['sub_category_id'])
        ...
        ////////////////////////////attribute/////////////////////////
        $attributeArr=array();
        $attribute = $this->db->select("attribute.*,am.is_primary")
                        ->where("am.sub_category_id",$data['sub_category_id'])
                        ->where("am.type",1)
                        ->where("attribute.status",1)
                        ->join("attribute_mapping as am", "am.attribute_id=attribute.attribute_id")
                        ->order_by("am.is_primary", "ASC")
        ...
        $specification = $this->db->select("attribute.*")
                        ->where("am.sub_category_id",$data['sub_category_id'])
                        ->where("am.type",2)
  ...
  function getBrandProduct($data){
        ...
        if($product_cart){
            foreach($product_cart as $value){
                ...
                if(isset($data['address_id']) and $data['address_id']){
                    $user_address = $this->db->select("user_address.*,dc.hub_id,dc.delivery_charge")
            ...
            $paybleAmount=$totalAmount+$delivery_amount;
            $remainAmount=$paybleAmount-$orderUpfront;
            return array('status'=>true,'cart'=>$cartArr,'total_amount'=>$totalAmount,'delivery_amount'=>$delivery_amount,'payble_amount'=>$paybleAmount,'upfront_amount'=>$orderUpfront,'remain_amount'=>$remainAmount);
        }else{
            return array('status'=>false,'cart'=>array(),'total_amount'=>"0",'delivery_amount'=>"0",'payble_amount'=>"0",'upfront_amount'=>"0",'remain_amount'=>"0");
        }
    }
    ...
    function filterData($data){
        ...
        $attrWhere='products.category_id="'.$data['category_id'].'" and products.sub_category_id="'.$data['sub_category_id'].'" and products.status=1 and pag.group_id=0';
...
function orderDetail($data){
    ...
    if($orders){
        ...
        $getOrderItems=array();
        $order_items = $this->db->select('ot.*,p.name as product_name,c.name as category_name')
...
function couponList($data){
    $couponArr=array();
    $currDate=date('Y-m-d');
    $couponData=$this->getTableDataArray('coupon','status="1" and (start_date<="'.$currDate.'" and end_date>="'.$currDate.'")');
    //print_r($coupon);exit;
    $getMyCart=$this->getMyCart($data);
...
function applyCoupon($data){
    ...
    if($coupon and $getMyCart){
        ...
        if($isApply){
            ...
            return array('coupon_type'=>$coupon['coupon_type'],
                        'total_amount'=>$getMyCart['total_amount'],
