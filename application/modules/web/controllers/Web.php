...
class Web extends MX_Controller {
    ...
    function __construct() {
        ...
        if(isset($_SERVER['HTTP_REFERER']) and $_SERVER['HTTP_REFERER']){
            ...
            if($this->uri->segment('2')=='product-detail'){
                $product_name = $this->uri->segment('3');
                $product_id = $this->uri->segment('4');
                $item_id = $this->uri->segment('5');
                $ram = $this->uri->segment('6');
                $color = $this->uri->segment('7');
                echo "<script>window.location.href='".base_url('product-detail/'.$product_name.'/'.$product_id)."'</script>";
            }
        }
        ...
        if($this->language_data==$uriSegment){
            $this->uri->segment(1);
        }else{
            if($uriSegment!="web"){
                $this->config->set_item('base_url',base_url($this->language_data)) ;
                redirect(base_url());
            }
        }
...
//        echo "<pre/>";print_r($data);exit;
    ...
    public function search_product() {
        $language_data = $this->language_data;
        $search = urldecode(str_replace('%20', ' ', $this->uri->segment(3)));
    ...
    public function brand_list() {
        ...
        $brand = $this->User_model->getBrandDataArrayLimit('brand', 'status=1', '100', '0');
