...
class Home extends CI_Controller {
    ...
    function __construct() {
        ...
        if (!$this->session->userdata('admin_logged_in')) {
            ...
            if ($urlExceptions != 'ajax' && $urlExceptions!="admin_product_list_ajax") {
                if ($admin_login['type']) {
    ...
    public function add_new_product() {
        ...
        if (isset($_POST['add_product'])) {
            ...
            if (isset($_POST['is_express']) and $_POST['is_express']) {
...
//                                    $result = $reponse['imagepath'];
    ...
    public function admin_product_list_ajax(){

        $productListArr = array();

        $orderColumn=[null , null , "products.product_from" , "pag.item_no" , 'products.name' , 'pag.price' , 'pag.quantity'];
        $searchColumn = array( "products.product_from" , "pag.item_no" , 'products.name' , 'pag.price' , 'pag.quantity');

        $arrayList = []; 
        $filter_condition = [];
        $filterAttr = [];
        if ($this->input->get('category_id')!="" && $this->input->get('category_id')) {
            $filterAttr = $this->Admin_model->setFilterAttributes(['category_id' => $this->input->get('category_id') ]);
            $filter_condition['products.category_id'] = $this->input->get('category_id');
        }
        if ($this->input->get('sub_category_id')!="" && $this->input->get('sub_category_id')) {
            $filter_condition['products.sub_category_id'] = $this->input->get('sub_category_id');
        }
        if ($this->input->get('brand_id')!="" && $this->input->get('brand_id')) {
            $filter_condition['products.brand_id'] = $this->input->get('brand_id');
        }
        if ($this->input->get('model_id')!="" && $this->input->get('model_id')) {
            $filter_condition['products.model_id'] = $this->input->get('model_id');
        }
        if ($filter_condition) {
        ...
        $this->db->select("products.product_id,products.product_from,products.name,products.status,pag.item_id,pag.attribute_group_id,pag.item_no,pag.quantity,pag.discount,pag.price,pag.images,pc.name as category_name,psc.name as sub_category_name")
                        ->where("products.status!=", 99)
                        ...
                        if($this->input->get('order')  ){
                            $this->db->order_by($orderColumn[$this->input->get('order')['0']['column'] ] , $this->input->get('order')['0']['dir']);
                        }else {
        ...
        if ($products) {
            foreach ($products as $item) {
                ...
                $checkbox='
                <input  onchange="selectData(this,2)" value="'.$item['product_id'].'" name="cat_checker" type="checkbox"/>
                ';

                $image='
                <a href="'. base_url('admin/product-detail/' . $item['product_id']).'"><img src="'. $item['image'] .'" class="imgesClassTable"></a>
                        ';
                        
                $productFrom ='
                <span style="background: red;color: #fff;padding: 3px;border-radius: 6px;">'.$item['product_from'] == 2 ? "Dubai" : "Inventory".'</span>';        

                $itemNo=$item['item_no'];
                $productName='
                <a href="'. base_url('admin/product-detail/' . $item['product_id']).'" style="color: red;cursor: pointer;">'. $item['name'].'</a>
                ';
                $price= number_format(($item['price'] - (($item['price'] * $item['discount']) / 100)), 2);
                $price.=$item['discount'] > 0 ?'<p class="text-muted" style="text-decoration: line-through;">'. number_format($item['price'], 2).' </p>' : "";
                ...
                $status=$item['status'] != 0 ?
                    '<div class="mytoggle"><label class="switch"><input type="checkbox" onchange="checkStatus(this, '. $item['product_id'].')" '.$isChecked.' > <span class="slider round"></span></label></div>'
                : "" ;

                $action='
                    <a href="'. base_url('admin/edit-product-detail/' . $item['product_id']).'" class="composemail"><i class="fa fa-edit"></i></a>
                    <a href="#" onclick="deleteProduct(this, '. $item['product_id'] .');" class="composemail"><i class="fa fa-trash"></i></a>
                ';
        ...
        $this->db->select("products.product_id,products.product_from,products.name,products.status,pag.item_id,pag.attribute_group_id,pag.item_no,pag.quantity,pag.discount,pag.price,pag.images,pc.name as category_name,psc.name as sub_category_name")
                        ...
                        $allProductCount = $this->db->get("products")->num_rows();

		$output = array(
			"draw" 				=> $this->input->get('draw'),
			"recordsTotal" 		=> $allProductCount,
			"recordsFiltered"	=> $allProductCount,
			"data" 				=> $arrayList,
		);
    ...
    public function home_layout_product_ajax() {

            $orderColumn=[null , null , 'products.name' , 'pag.price' , 'pag.quantity'];
        ...
        if ($products) {
            foreach ($products as $item) {
                ...
                $my_deals_item          = $this->Admin_model->getDataResultRow('home_layout_item','home_layout_id="'.$this->input->get('uri').'" and product_id="'.$item['product_id'].'"');
                if($my_deals_item){
                    $item['is_exist']="1";
                }else{
                    $selectedValue=explode("@" ,$this->input->get('selectedValue') );
                    if (in_array($item['product_id'], $selectedValue)){
                        $item['is_exist']="1";
                    }else{
                        $item['is_exist']="0";
                    }
                ...
                $isChecked=$item['is_exist']==1 ? "checked" :"";

                $checkbox='
                <input '.$isChecked.' onchange="selectData(this,2,'.$item['product_id'].')" value="'.$item['product_id'].'" name="cat_checker" type="checkbox"/>
                ';
...
//////////////////////////////////////Home Layout Management////////////////////////////////////////////
    public function create_layout() {
        ...
        $category_list = $this->Admin_model->getDataResultArray('home_layout', 'status!=99', 'home_layout_id');
        if (isset($id)) {
            $edit = $this->Admin_model->getDataResultRow('home_layout', ['home_layout_id' => $id, 'status!=' => 99], '');
            ...
            if ($edit) {
                ...
                if ($this->form_validation->run() == False) {
                    ...
                    $data['view_link'] = 'home_layout/home_layout';
                    $this->load->view('layout/template', $data);
                } else {
                    $updated_images = $this->update_img('image', 'uploads/', $edit['image']);
                    $update = [
                        'name' => $this->input->post('english'),
                        'image' => $updated_images,
                    ];
                    //  echo '<pre>'; print_r($update);exit;
                    $query = $this->Admin_model->updatedataTable('home_layout', ['home_layout_id' => $id], $update);
                    if ($query) {
            ...
            if ($this->form_validation->run() == False) {
                ...
                $returnData = $this->Admin_model->insertData('home_layout', $insertArr);
                if ($returnData) {
    ...
    public function home_layout_product() {
        $uri=$this->uri->segment(3);
        $edit = $this->Admin_model->getDataResultRow('home_layout', ['home_layout_id' => $uri, 'status!=' => 99], '');
        if($edit){
            $selected="";
            ...
            if($products){
                foreach($products as $item){
                    $my_deals_item          = $this->Admin_model->getDataResultRow('home_layout_item','home_layout_id="'.$uri.'" and product_id="'.$item['product_id'].'"');
                    if($my_deals_item){
                        if($selected==''){
            ...
            $data['view_link'] = 'home_layout/home_layout_product';
            $this->load->view('layout/template', $data);
        }else{
            echo "<script>alert('Layout not found.');window.location.href='" . base_url() . "admin/create-layout';</script>";
        }
