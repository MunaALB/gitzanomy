...
class Order extends MX_Controller {
    ...
    function new_order_list() {
        ...
        ///////////////////
        $whereCondOrder=" ( status=1 or status=2 or status=3 or status=4) ";
        if(isset($_POST['order_status']) and $_POST['order_status']){
            $data['order_status']=$_POST['order_status'];
            $whereCondOrder=$whereCondOrder.' and status="'.$_POST['order_status'].'"';
        }
        $order = $this->getTableDataArray('orders', $whereCondOrder);
        if ($order) {
            foreach ($order as $list) {
                ...
                $isFilter=false;
                $whereCond=" ot.order_id ='".$list['order_id']."'";
                if(isset($_POST['from']) and $_POST['from']){
                    $data['from']=$_POST['from'];
                    $isFilter=true;
                    if($_POST['from']==3){
                        $whereCond=$whereCond.' and p.product_from="0"';
                    }else{
                        $whereCond=$whereCond.' and p.product_from="'.$_POST['from'].'"';
                    }
                }

                if(isset($_POST['item_status']) and $_POST['item_status']){
                    $data['item_status']=$_POST['item_status'];
                    $isFilter=true;
                    if($_POST['item_status']==5){
                        $whereCond=$whereCond.' and ot.item_action="0"';
                    }else{
                        $whereCond=$whereCond.' and ot.item_action="'.$_POST['item_status'].'"';
                    }
                }
                $dataExist=false;
                if($isFilter){
                    $orderItems = $this->db->select('ot.*,p.name as product_name,c.name as category_name,p.product_from')
                            ->where($whereCond)
                            ->from('order_items ot')
...
//            $filter_condition = ['vendor_id' => $_POST['vendor_id']];
        ...
        $order = $this->getTableDataArray('orders', ' ( status=1 or status=2 or status=3 or status=4) ');
        if ($order) {
            foreach ($order as $list) {
                $users = $this->getSingleDataRow('users', 'user_id="' . $list['user_id'] . '"');
                if ($users) {
    ...
    function new_order_detail() {
        $internationalArr=array();
        $inventoryArr=array();
        $vendorArr=array();
        $order_id = $this->uri->segment(3);
        $orderArr = array();
        $isAnyVendorProduct = 0;
        $order = $this->getSingleDataRow('orders', 'order_id="' . $order_id . '"');
        ...
        if ($order) {
            ...
            if ($orderItems) {
                foreach ($orderItems as $items) {
                    ...
                    if ($items['driver_id'] > 0) {
                        ...
                        if ($order_status) {
                            foreach ($order_status as $stats) {
                                ...
                                if ($getCheck) {
                                    $stats['is_checked'] = "1";
                                    $stats['tracker_date'] = $getCheck['vendor_tracking_created_at'];
                                } else {
                                    $stats['is_checked'] = "0";
                                    $stats['tracker_date'] = "";
                                }
                    ...
                    $vndr_driver_order_get = $this->getSingleDataRow('driver_order', 'order_id="' . $order['order_id'] . '" and driver_id="' . $items['driver_id'] . '" and driver_tracking_id="' . $items['vendor_tracking_id'] . '" ');
                    if($vndr_driver_order_get){
                        $items['vendor_driver_order']=$vndr_driver_order_get;
                    }else{
                        $items['vendor_driver_order']=$vndr_driver_order_get;
                    }
                    //////////////////////////////////Vendor Assigned TRACKER////////////////////////////

                    //////////////////////////////////International Assigned TRACKER////////////////////////////
                    $vendorItemStatusArr = array();
                    if ($items['international_driver_id'] > 0) {
                        $order_status = $this->getTableDataArray('order_status', 'type=3 and (order_type_id=4) ');
                        if ($order_status) {
                            foreach ($order_status as $stats) {
                                $getCheck = $this->getSingleDataRow('order_international_tracking', 'order_id="' . $order['order_id'] . '" and driver_id="' . $items['international_driver_id'] . '" and international_tracking_id="' . $items['international_tracking_id'] . '" and international_status="' . $stats['status_id'] . '"');
                                // echo '<pre/>';print_r($getCheck);exit;
                                if ($getCheck) {
                                    $stats['is_checked'] = "1";
                                    $stats['tracker_date'] = $getCheck['international_tracking_created_at'];
                                } else {
                                    $stats['is_checked'] = "0";
                                    $stats['tracker_date'] = "";
                                }
                        ...
                        $driver_address = $this->getSingleDataRow('driver', 'driver_id="' . $items['international_driver_id'] . '" ');
                        if ($driver_address) {
                            unset($driver_address['password']);
                            $items['internatioanl_driver_detail'] = $driver_address;
                        } else {
                            $items['internatioanl_driver_detail'] = array();
                        }
                        $items['international_item_tracking'] = $this->getTableDataArray('order_international_tracking', 'order_id="' . $order['order_id'] . '" and driver_id="' . $items['international_driver_id'] . '" and international_tracking_id="' . $items['international_tracking_id'] . '"');
                    }
                    $items['international_item_status'] = $vendorItemStatusArr;
                    $items['international_item_status_count'] = count($vendorItemStatusArr);
                    $international_driver_order_get = $this->getSingleDataRow('driver_order', 'order_id="' . $order['order_id'] . '" and driver_id="' . $items['international_driver_id'] . '" and driver_tracking_id="' . $items['international_tracking_id'] . '" ');
                    if($international_driver_order_get){
                        $items['international_driver_order']=$international_driver_order_get;
                    }else{
                        $items['international_driver_order']=$international_driver_order_get;
                    }
                    //////////////////////////////////International Assigned TRACKER////////////////////////////

                    //////////////////////////////////User Assigned TRACKER////////////////////////////
                    $vendorItemStatusArr = array();
                    if ($items['drop_tracking_id'] > 0) {
                        ...
                        $driver_address = $this->getSingleDataRow('driver', 'driver_id="' . $items['drop_tracking_id'] . '" ');
                        if ($driver_address) {
                            unset($driver_address['password']);
                    ...
                    $items['drop_item_status'] = $vendorItemStatusArr;
                    $items['drop_item_status_count'] = count($vendorItemStatusArr);
                    $drop_driver_order_get = $this->getSingleDataRow('driver_order', 'order_id="' . $order['order_id'] . '" and driver_id="' . $items['drop_driver_id'] . '" and driver_tracking_id="' . $items['drop_tracking_id'] . '" ');
                    if($drop_driver_order_get){
                        $items['drop_driver_order']=$drop_driver_order_get;
                    }else{
                        $items['drop_driver_order']=$drop_driver_order_get;
                    }
                    //////////////////////////////////User Assigned TRACKER////////////////////////////
                    if($items['product_from']==1){
                        array_push($inventoryArr, $items);
                    }elseif($items['product_from']==2){
                        array_push($internationalArr, $items);
                    }else{
                        array_push($vendorArr, $items);
                    }
            ...
            $order['inventory_items'] = $inventoryArr;
            $order['international_items'] = $internationalArr;
            $order['vendor_items'] = $vendorArr;
            ...
            $driver_order_get = $this->getSingleDataRow('driver_order', 'order_id="' . $order['order_id'] . '" and driver_id="' . $order['driver_id'] . '" and driver_tracking_id="' . $order['upfront_tracking_id'] . '" ');
            if($driver_order_get){
                $order['driver_order']=$driver_order_get;
            }else{
                $order['driver_order']=$driver_order_get;
            }
            //echo '<pre/>';print_r($order);exit;
            ...
            if($order['status']==1){
                if($order['upfront_amount']>0){
                    ///Upfront Order
                    // echo "Upfront Order";exit;
                    $data['view_link'] = 'orders/upfront_order_detail';
                    $this->load->view('layout/template', $data);
                }elseif($order['isAnyVendorProduct']){
                    //any vendor product
                    // echo "any vendor product";exit;
                    $data['view_link'] = 'orders/get_an_order_detail';
                    $this->load->view('layout/template', $data);
                }else{
                    // only admin product
                    // echo "only admin product";exit;
                    $data['view_link'] = 'orders/ready_for_delivery';
                    $this->load->view('layout/template', $data);
                }
            ...
            echo '<script>alert("No order found.");window.location.href="'.base_url('admin/new-order-list').'";</script>';
        }
        // echo '<pre/>';print_r($data);exit;
    ...
    function return_order_list() {
        ...
        $orderArr = array();
        $whereCondOrder=" ( status=1 or status=2 or status=3 or status=4 or status=5 or status=6) ";
        $order = $this->getTableDataArray('orders', $whereCondOrder);
        if ($order) {
            foreach ($order as $list) {
                $users = $this->getSingleDataRow('users', 'user_id="' . $list['user_id'] . '"');
                ...
                $isFilter=false;
                $whereCond=" ot.order_id ='".$list['order_id']."' and ot.return_reason!=''";
                if(isset($_POST['from']) and $_POST['from']){
                    $data['from']=$_POST['from'];
                    $isFilter=true;
                    if($_POST['from']==3){
        ...
        $data['view_link'] = 'orders/return_order_list';
        $this->load->view('layout/template', $data);
    }
        ...
        if ($order) {
            ...
            if ($orderItems) {
                foreach ($orderItems as $items) {
                    ...
                    //////////////////////////////////Get Return Item On hub from user location////////////////////////////
                    $vendorItemStatusArr = array();
                    if ($items['return_driver_id'] > 0) {
                        $order_status = $this->getTableDataArray('order_status', 'type=3 and (order_type_id=5) ');
                        if ($order_status) {
                            foreach ($order_status as $stats) {
                                $getCheck = $this->getSingleDataRow('order_return_tracking', 'order_id="' . $order['order_id'] . '" and driver_id="' . $items['return_driver_id'] . '" and return_tracking_id="' . $items['return_tracking_id'] . '" and return_status="' . $stats['status_id'] . '"');
                                // echo '<pre/>';print_r($getCheck);exit;
                                if ($getCheck) {
                                    $stats['is_checked'] = "1";
                                    $stats['tracker_date'] = $getCheck['return_tracking_created_at'];
                                } else {
                                    $stats['is_checked'] = "0";
                                    $stats['tracker_date'] = "";
                                }
                        ...
                        $driver_address = $this->getSingleDataRow('driver', 'driver_id="' . $items['return_driver_id'] . '" ');
                        if ($driver_address) {
                            unset($driver_address['password']);
                            $items['drop_driver_detail'] = $driver_address;
                        } else {
                        ...
                        $items['drop_item_tracking'] = $this->getTableDataArray('order_return_tracking', 'order_id="' . $order['order_id'] . '" and driver_id="' . $items['return_driver_id'] . '" and return_tracking_id="' . $items['return_tracking_id'] . '"');
                    }
                    $items['drop_item_status'] = $vendorItemStatusArr;
                    $items['drop_item_status_count'] = count($vendorItemStatusArr);
                    $drop_driver_order_get = $this->getSingleDataRow('driver_order', 'order_id="' . $order['order_id'] . '" and driver_id="' . $items['return_driver_id'] . '" and driver_tracking_id="' . $items['return_tracking_id'] . '" ');
                    if($drop_driver_order_get){
                        $items['drop_driver_order']=$drop_driver_order_get;
                    }else{
                        $items['drop_driver_order']=$drop_driver_order_get;
                    ...
                    //////////////////////////////////DROP Return Item On hub To vendor location////////////////////////////
                    $vendorDropItemStatusArr = array();
                    if ($items['return_driver_id'] > 0) {
                        $return_order_status = $this->getTableDataArray('order_status', 'type=3 and (order_type_id=6) ');
                        if ($return_order_status) {
                            foreach ($return_order_status as $stats) {
                                $getCheck = $this->getSingleDataRow('order_return_drop_tracking', 'order_id="' . $order['order_id'] . '" and driver_id="' . $items['return_drop_driver_id'] . '" and return_drop_tracking_id="' . $items['return_drop_tracking_id'] . '" and return_drop_status="' . $stats['status_id'] . '"');
                                // echo '<pre/>';print_r($getCheck);exit;
                                if ($getCheck) {
                                    $stats['is_checked'] = "1";
                                    $stats['tracker_date'] = $getCheck['return_drop_tracking_created_at'];
                                } else {
                                    $stats['is_checked'] = "0";
                                    $stats['tracker_date'] = "";
                                }
                                array_push($vendorDropItemStatusArr, $stats);
                            }
                        }
                        $return_driver_address = $this->getSingleDataRow('driver', 'driver_id="' . $items['return_drop_driver_id'] . '" ');
                        if ($return_driver_address) {
                            unset($return_driver_address['password']);
                            $items['return_drop_driver_detail'] = $return_driver_address;
                        } else {
                            $items['return_drop_driver_detail'] = array();
                        }
                        $items['return_drop_item_tracking'] = $this->getTableDataArray('order_return_drop_tracking', 'order_id="' . $order['order_id'] . '" and driver_id="' . $items['return_driver_id'] . '" and return_drop_tracking_id="' . $items['return_drop_tracking_id'] . '"');
                    }
                    $items['return_drop_item_status'] = $vendorDropItemStatusArr;
                    $items['return_drop_item_status_count'] = count($vendorDropItemStatusArr);
                    $return_drop_driver_order_get = $this->getSingleDataRow('driver_order', 'order_id="' . $order['order_id'] . '" and driver_id="' . $items['return_drop_driver_id'] . '" and driver_tracking_id="' . $items['return_drop_tracking_id'] . '" ');
                    if($return_drop_driver_order_get){
                        $items['return_drop_driver_order']=$return_drop_driver_order_get;
                    }else{
                        $items['return_drop_driver_order']=$return_drop_driver_order_get;
                    }
                    //////////////////////////////////DROP Return Item On hub To vendor location////////////////////////////
            ...
            // echo '<pre/>';print_r($vendorArr);exit;
            $order['isAnyVendorProduct'] = $isAnyVendorProduct;
            $order['order_items'] = $getOrderItems;
            $order['inventory_items'] = $inventoryArr;
            $order['international_items'] = $internationalArr;
            ...
            $data['order'] = $order;
            // echo '<pre/>';print_r($data);exit;
            $data['view_link'] = 'orders/return_order_detail';
            $this->load->view('layout/template', $data);
        } else {
            echo '<script>alert("No order found.");window.location.href="'.base_url('admin/new-order-list').'";</script>';
        }
