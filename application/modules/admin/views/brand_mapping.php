            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (jsonData['error_code'] == 100) {
                        ...
                        var html = '<label class="d-block">Brands</label>';
                        var isDataAvailable = false;
                        $(jsonData).each(function (i, v) {
                            isDataAvailable = true;
                            //alert(v['sub_category_id'])
                            // html += '<li style="cursor:pointer;" onclick="deleteData(this,' + v['brand_id'] + ');" class="btn btn-rounded btn-secondary">' + v['name'] + '</li>';
                            html += '<li style="cursor:pointer;" onclick="deleteData(this,' + v['brand_mapping_id'] + ');" class="btn btn-rounded btn-secondary">' + v['name'] + '</li>';
                        });
                        if (isDataAvailable) {
        ...
        if (check) {
            if (id) {
                $.ajax({
                    ...
                    type: 'post',
                    data: 'method=ModelStatus&id=' + id + '&action=99&type=2',
                    success: function (data) {
                        var dt = $.trim(data);
                        //alert(dt);
