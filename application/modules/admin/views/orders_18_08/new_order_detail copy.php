...
<div class="content-wrapper">
                            ...
                            if($order['status']==1){
                                ...
                                if($order['upfront_amount']>0){
                                    ...
                                    if($order['isAnyVendorProduct']){
                                        ...
                                        // only admin product
                                        $assignedUpfrontDriverList=2;
                                        $upfrontButton='<button type="button" class="btn btn-secondary">Assign of Upfront</button>';
                                        $getFromVendor='<button type="button" class="btn btn-secondary ">Get an item</button>';
                                        $readyForDelivery='<button type="button" class="btn btn-secondary active">Ready for Delivery</button>';
                                    }
                                    
                                }
                            }elseif($order['status']==2){
                                //Out For Collecting Cash
                                $assignedUpfrontDrivertracker=1;
                                $upfrontButton='<button type="button" class="btn btn-secondary active">Assign of Upfront</button>';
                                $getFromVendor='<button type="button" class="btn btn-secondary">Get an item</button>';
                                $readyForDelivery='<button type="button" class="btn btn-secondary">Ready for Delivery</button>';
                                    ...
                                    <div class="card-body">
                                        <?php if(isset($order['order_items']) and $order['order_items']): foreach ($order['order_items'] as $key => $items): ?>
                                            <div class="card m-b-3 order-productss">
                                                <div class="card-body">
                                                    <div class="row">
                                                        <input type="checkbox" onchange="checkOrderData(this,'<?=$order['order_id'];?>');" class="item_class item_class_css" value="<?=$items['order_item_id'];?>" name="select_item">
                                                    </div>
                                                </div>
                                                <div class="card-body">
                                                    <div class="row">
    ...
    <div class="asiign-driver-list" id="listdriveruseritem" style='display:block'>
        <div class="row">
            <div class="col-lg-12 m-b-3">
                <div class="card">
                    <div class="card-body">
                        <h5>
                            Assign Driver
                            <div class="assignDriverSaved">
                            <button type="button" id="assignForUserPorduct" onclick="assignForUserPorduct(this,'<?=$order['order_id'];?>');" class="btn btn-secondary active btn-new">Save</button>
                            </div>
                            <span class="pull-right f-13">
                                <a href="javascript:void(0)" id="remove-driverlist"><i class="fa fa-times"></i></a>
                            </span>
                        ...
                        <?php if($driver_list): ?>
                            <div class="driverilist-side">
                                <?php foreach($driver_list as $list): ?>
                                    <div class="message-widget" onclick="selectDriverUserItem(this,'<?=$order['order_id'];?>','<?=$list['driver_id'];?>')">
                                        <a href="#">
                                            <div class="user-img pull-left">
                                                <?php if($list['image']): ?>
                                                    <img src="<?=$list['image'];?>" class="img-circle img-responsive" alt="User Image" />
    ...
    <script type="text/javascript">
        ...
        function driverListShow(o,t,oid){
            $("#assignDriverSaved").empty();
            $("#assignDriverSaved").append('');
            $("#listdriver").addClass("active");
        }

        function driverListShowForUserProduct(o,t,oid){
            $("#assignDriverSaved").empty();
            $("#assignDriverSaved").append('');
            $("#listdriveruseritem").addClass("active");
        }
    </script>
    ...
    <script>
        ...
        function selectDriverUserItem(o,i,d){
            $(".message-widget").removeClass('set-active-driver');
            $(o).addClass('set-active-driver');
            $("#assignForUserPorduct").removeClass('btn-new');
            $("#assignForUserPorduct").addClass('btn-new-active');
            $("#assignForUserPorduct").attr('data-driver',d);
        }
        ...
        function checkOrderData(o,oid){
            ...
            if(select_item==undefined || select_item=='undefined'){
                ...
                $("#assignedByDriverDiv").empty();
                $("#assignedByDriverDiv").append('<a href="javascript:void(0)" onclick="driverListShowForUserProduct(this,2,\''+oid+'\');" class="btn btn-secondary activeButton">Assign a Driver</a>');
            }
        }

        function assignForUserPorduct(obj,orderId) {
            var newData='';
            var driver_id=$('#assignForUserPorduct').attr('data-driver');
            if(driver_id==undefined || driver_id=='undefined'){
                alert("Please select a driver.");
            }else{
                if (orderId) {
                    ...
                    $.ajax({
                        url: "<?= base_url(); ?>admin/Order/ajax_method",
                        type: 'post',
                        data: 'method=assignForUserPorduct&driverId=' + driver_id + '&orderId=' + orderId+ '&itemId=' + newData,
                        success: function (data) {
                            var dt = $.trim(data);
                            var jsonData = $.parseJSON(dt);
                            if (jsonData['error_code'] == "100") {
