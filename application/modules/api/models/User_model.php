...
function sendSMSForSNS($numbers, $msg, $otp) {
    ...
    $message = $msg;
    $urls = "https://control.msg91.com/api/sendotp.php?authkey=8369AupDatZgXo5f4d0766P123&mobile=" . $mobileNumber . "&message=" . $message . "&sender=EVTOTP&otp=" . $otp . "";
...
//                print_r($productAttribute);EXIT;
    ...
    function homepage_31_10($data,$langData){
    ...
    function homepage($data,$langData){
        ...
        //print_r($topBookingService);exit;
        $home_layoutArr=array();
        $this->db->where('status',1);
        $this->db->limit(5);
        $this->db->order_by('home_layout_id','DESC');
        $home_layout = $this->db->get('home_layout')->result_array();
        if($home_layout){
            foreach($home_layout as $layout){
                $home_layoutProductsArr=array();
                $home_layoutProducts =  $this->db->select("products.*,pag.item_id,pag.attribute_group_id,pag.item_no,pag.price,pag.discount,pag.quantity,pag.images")
                            ->where("products.status",1)
                            ->where("pag.group_id",0)
                            ->where("pag.item_status",1)
                            ->where("hli.home_layout_id",$layout['home_layout_id'])
                            ->join("product_attribute_group as pag", "pag.product_id=products.product_id")
                            ->join("home_layout_item as hli", "hli.product_id=products.product_id")
                            ->limit(10,0)
                            ->get("products")->result_array();
                        // print_r($home_layoutProducts);exit;
                if($home_layoutProducts){
                    $home_layoutProductsArr=$this->getProductDataResult($home_layoutProducts,$data,$langData);
                    $layout['product_list']=$home_layoutProductsArr;
                    array_push($home_layoutArr,$layout);
                }
            }
        }
        ...
        // $this->db->limit(10);
        // $home_layout = $this->db->get('product_sub_category')->result_array();
        
        $subCategoryProductArr=array();
        $getSubCategory = $this->db->select("product_sub_category.*")
                        ->where("product_sub_category.status",1)
                        ->where("product_sub_category.home",1)
                        ->where("psc.status",1)
                        ->join("products as psc", "psc.sub_category_id=product_sub_category.sub_category_id")
                        ->limit(5)
                        ->group_by('psc.sub_category_id')
                        ->get("product_sub_category")->result_array();

        if($getSubCategory){
            foreach($getSubCategory as $layout){
                $home_layoutProductsArr=array();
                $home_layoutProducts =  $this->db->select("products.*,pag.item_id,pag.attribute_group_id,pag.item_no,pag.price,pag.discount,pag.quantity,pag.images")
                            ->where("products.status",1)
                            ->where("pag.group_id",0)
                            ->where("pag.item_status",1)
                            ->where("products.sub_category_id",$layout['sub_category_id'])
                            ->join("product_attribute_group as pag", "pag.product_id=products.product_id")
                            ->limit(10,0)
                ...
                if($home_layoutProducts){
                    $home_layoutProductsArr=$this->getProductDataResult($home_layoutProducts,$data,$langData);
                    $layout['product_list']=$home_layoutProductsArr;
                    array_push($subCategoryProductArr,$layout);
                }
            }
        }
        // print_r($home_layoutArr);exit;

        return array('webslider'=>$webslider,'slider'=>$slider,'productCategory'=>$productCategoryArr,'mostViewsProducts'=>$mostViewsProductsArr,'topSellingProducts'=>$topSellingProductsArr,'serviceCategory'=>$serviceCategory,'mostBooking'=>$topBookingServiceArr,'user_status'=>$userStatus,'home_layoutArr'=>$home_layoutArr,'subCategoryProductArr'=>$subCategoryProductArr);
    ...
    function getCategorySubCategory($data,$langData){
        ...
        if($brand){
            foreach ($brand as $brandData) {
                $checkProduct=$this->getTableDataArray('products','brand_id="'.$brandData['brand_id'].'" and category_id="'.$data['category_id'].'" and sub_category_id="'.$data['sub_category_id'].'" and status="1"');
                if($checkProduct){
                    array_push($brandArr,$brandData);
                }
            }
        }
        ////////////////////////////Brand/////////////////////////
        ...
        $attribute = $this->db->where("am.sub_category_id",$data['sub_category_id'])
                        ...
                        ->where("attribute.status",1)
                        ->join("attribute_mapping as am", "am.attribute_id=attribute.attribute_id")
                        // ->join("product_attribute as p", "attribute.attribute_id=p.attribute_id")
                        ->order_by("am.is_primary", "ASC")
                        // ->group_by('attribute.attribute_id')
                        ->get("attribute")->result_array();
            if($attribute){
                ...
                foreach($attribute as $attr){
                    ...
                    $value = $this->db->select('attribute_value.*')
                        ...
                        ->get("attribute_value")->result_array();
                    if($value){
                        foreach($value as $attrValues){
                            $cat_attrVal=$this->getSingleDataRow('product_attribute','sub_category_id="'.$data['sub_category_id'].'" and attribute_value_id="'.$attrValues['attribute_value_id'].'"');
                            if($cat_attrVal){
                                array_push($attributeValues,$attrValues);
                            }
                        }
                    }
                        

                    $attr['attribute_value']=$attributeValues;
                    $isAttrProduct=$this->getSingleDataRow('product_attribute','sub_category_id="'.$data['sub_category_id'].'" and attribute_id="'.$attr['attribute_id'].'"');
                    if($isAttrProduct){
        ...
        ////////////////////////////Specification/////////////////////////
        $filter_data=array('categoryList'=>$catArr,'brand'=>$brandArr,'attribute'=>$attributeArr,'specification'=>$specificationArr,'min_price'=>'10','max_price'=>'40000');
...
//                        ->limit($data['limit'],$data['start'])
  ...
  function getBrandProduct($data,$langData){
    ...
    function searchProduct($data,$langData){
        ...
        $dbLike="products.status=1 and pag.group_id=0 and pag.item_status=1 and (products.name LIKE '%".$data['search']."%' or products.name_ar LIKE '%".$data['search']."%' )";
        $featuredProducts = $this->db->select("products.*,pag.item_id,pag.attribute_group_id,pag.item_no,pag.price,pag.discount,pag.quantity,pag.images")
                            ->where($dbLike)
                            ->join("product_attribute_group as pag", "pag.product_id=products.product_id")
//                            ->limit($data['limit'],$data['start'])
    ...
    function getMyCart($data,$langData){
        ...
        if($product_cart){
            ...
            $remainAmount=$paybleAmount+$delivery_amount;
    ...
    function placeOrder_paymet_gateway($data){
        $getMyCart=$this->getMyCart($data,'en');
        $expected_delivery=$getMyCart['expected_delivery'];
        if(isset($data['coupon_id']) and $data['coupon_id']){
            $applyCoupon=$this->applyCoupon($data);
...
function myOrders($data){
    ...
    $activeOrdersList=$this->getTableDataArrayOrderBy('orders','user_id="'.$data['user_id'].'" and status!=0 and ( user_status=1 or user_status=2  )','order_id');
    if($activeOrdersList){
    ...
    $completedOrdersList=$this->getTableDataArrayOrderBy('orders','user_id="'.$data['user_id'].'" and status!=0 and ( user_status=3 )','order_id');
